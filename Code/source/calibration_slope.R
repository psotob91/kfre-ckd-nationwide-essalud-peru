...
calibration_slope <- function(data, horizon, primary_event, pred) {
  ...
  return(data.frame(estimate = summary(fit_cal_slope)$mean["cll_pred", "estimate"], 
                    se = summary(fit_cal_slope)$mean["cll_pred", "san.se"], 
                    var = summary(fit_cal_slope)$mean["cll_pred", "san.se"] ^ 2))
}